...
class OlmManager {
    ...
    try {
      ...
      if (oldKeyCount != null) {
        ...
        if (oneTimeKeysCount > 0) {
          _olmAccount!.generate_one_time_keys(oneTimeKeysCount as int);
        }
        uploadedOneTimeKeysCount = oneTimeKeysCount + oldOTKsNeedingUpload as int?;
  ...
  ToDeviceEvent _decryptToDeviceEvent(ToDeviceEvent event) {
    ...
    final Future<Null> Function([OlmSession]) updateSessionUsage = ([OlmSession? session]) => runInRoot(() async {
          if (session != null) {
  ...
  Future<void> startOutgoingOlmSessions(List<DeviceKeys?> deviceKeys) async {
    ...
    final response = await client!.claimKeys(requestingKeysFrom as Map<String, Map<String, String>>, timeout: 10000);
